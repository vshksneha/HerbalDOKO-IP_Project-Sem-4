...
public class ChangeUserInfoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // set previous data
        etLName.setText(userInformation.getFirstName());
        etFName.setText(userInformation.getLastName());
            ...
            public void onFocusChange(View view, boolean onFocus) {
                if (onFocus) {
                    ...
                    if (etFName.getText().toString().equals("")) {
                        layoutFName.setHelperText("Name cannot be left blank");
                    } else if (!includeCharInAlphabet(etFName.getText().toString())) {
                        layoutFName.setHelperText("The name must contain at least 1 alphabetic character");
                    } else {
                        layoutFName.setHelperTextEnabled(false);
                    }
                }
            ...
            public void onFocusChange(View view, boolean onFocus) {
                if (onFocus) {
                    ...
                    if (etLName.getText().toString().equals("")) {
                        ...
                        layoutLName.setHelperText("They must contain at least 1 alphabetic character");
                    } else {
                        layoutLName.setHelperTextEnabled(false);
                    }
                }
        ...
        etPhone.setOnFocusChangeListener((view, onFocus) -> {
            if (onFocus) {
                layoutPhone.setHelperTextEnabled(false);
            } else {
                if (etPhone.getText().toString().equals("")) {
                    layoutPhone.setHelperText("Phone number cannot be left blank");
                } else if (etPhone.getText().toString().length() < 6) {
                    layoutPhone.setHelperText("The phone number must have at least 6 digits");
                } else {
                    layoutPhone.setHelperTextEnabled(false);
                }
            }
        ...
        btnChange.setOnClickListener(view -> {
            if (!validateData()) {
                            ...
                            .update(data).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        Toast.makeText(ChangeUserInfoActivity.this, "Successfully updated", Toast.LENGTH_SHORT).show();
                                        onBackPressed();
                                    } else {
                ...
                Toast.makeText(ChangeUserInfoActivity.this, "You have not entered complete information", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        public void onDateSet(DatePicker view, int year, int month, int day) {
            ...
            try {
                ...
                if (currentTime - datePicker <= 0) {
                    Toast.makeText(getActivity(), "You have just selected a date in the future", Toast.LENGTH_SHORT).show();
